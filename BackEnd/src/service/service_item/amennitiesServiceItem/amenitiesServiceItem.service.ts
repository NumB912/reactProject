...
export class AmenitesServiceItems {
  static async EditAmenityServiceItems(
    ...
    try {
      ...
      const existingRecords = await tx.amenitiesServiceItems.findMany({
      ...
      if (toDelete.length > 0) {
        delete_amenity =await tx.amenitiesServiceItems.deleteMany({
          where: {
      ...
      if (toInsert.length > 0) {
        ...
        add_amenity =await tx.amenitiesServiceItems.createMany({
          data: dataToInsert,
          skipDuplicates: true,
        });
      }
