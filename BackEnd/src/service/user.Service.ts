...
import type { EditProfileDTO } from "@/model/user/edit.profile.js";
...
export class userService {
  ...
  static async becomeSupplier(becomeSupplier: BecomeSupplierDTO):Promise<{
  ...
  static async handleFavorite(user_id:string, service_id:string): Promise<{success:boolean,message:string,status:number}> {
    ...
    const service = await prisma.service.findFirst({
      // where: { id: service_id, status_id: StatusType.PEDDING },
      select: { id: true },
    });
