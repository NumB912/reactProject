...
import ServiceDetailModal from "./service_modal";
import { statusService } from "../../enum/status_service.enum";
import AddServiceModal from "./add_service";
...
const ServiceManagementSupplier = () => {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">
            Quản lý tất cả dịch vụ được cung cấp trên hệ thống
          </p>
        </div>
      ...
      <div className="flex w-full justify-end mb-3 ">
         <button className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center" onClick={()=>{
          handleOpenAddModal()
         }}>
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-gray-600 text-sm mt-1">
                  Nhà cung cấp: {service?.supplier.name}
                  Công ty:{" "}
                  {service.supplier.Request_become_supplier?.company_name}
                </p>
              </div>
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${
                  service?.status_service.statusService=== statusService.ACTIVE
                    ? "bg-green-100 text-green-800"
                    : "bg-red-100 text-red-800"
                }`}
              >
                {service?.status_service.statusService === statusService.ACTIVE ? "Hoạt động" : "Ngừng cung cấp"}
              </span>
            </div>
...
export default ServiceManagementSupplier;
