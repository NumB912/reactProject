...
export const useCalendarCarStore = create<CarCalendarStore>((set, get) => ({
  datesPickUp: DaysOfMonth(currentMonth,currentYear),
  datesDropOff: DaysOfMonth(tomorrowMonth, tomorrowYear),
  ...
  nextMonthDatesPickUp: () => {
    ...
    const nextMonthDates = DaysOfMonth(next.getMonth()+1, next.getFullYear());
    set({
  ...
  prevMonthDatesPickUp: () => {
    const { datesPickUp } = get();
    const current = datesPickUp[0];
    const prev = new Date(current.getFullYear(), current.getMonth(), current.getDate());
    console.log(current.toLocaleDateString('vi-vn'))
  ...
  nextMonthDatesDropOff: () => {
    ...
    const next = new Date(current.getFullYear(), current.getMonth() + 1, current.getDate());
    const nextmonth = DaysOfMonth(next.getMonth()+1, next.getFullYear());
    set({
  ...
  prevMonthDatesDropOFf: () => {
    ...
    const prevMonthDates = DaysOfMonth(prev.getMonth()-1, prev.getFullYear());
    set({
