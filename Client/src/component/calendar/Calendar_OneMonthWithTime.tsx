  ...
  onSelected: (date: Date) => void,
    ...
    <>
      <div className="Calendar_rentalCar">
        <div className="dropTime flex justify-around items-center border-b-2 border-gray-300 pb-3 my-3">
          <div className="font-semibold">{titleTypeSeletedDate}</div>
          <div
            className="time border p-3 border-gray-300 relative w-[110px]"
            onClick={(e) => {
              e.stopPropagation();
          ...
          >
            ...
            >
              <TimeSelected
                ...
                setTime={(time) => {setSelectTime(time)}}
              />
            </div>
          </div>
        </div>
    ...
    const renderDayOneMonth = (
      ...
      return Array.from({ length: 42 }).map((_, index) => {
        ...
        const isab = date && dateStartSelected && dateEndSelected && isBetween(date, dateStartSelected, dateEndSelected)
        return (
          <div
            key={index}
            className={`day 
                ...
                ${date && dateStartSelected && dateEndSelected && isBetween(date, dateStartSelected, dateEndSelected) ? "bg-gray-300 text-white":""}
              `}
            onClick={() => {
